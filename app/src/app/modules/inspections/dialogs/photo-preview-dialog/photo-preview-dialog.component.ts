...
@Component({
  ...
  template: `
    <div class="flex flex-col bg-white overflow-hidden rounded-2xl">
      ...
      <div
        [class]="
          'p-4 flex items-center justify-center min-h-[300px] max-h-[70vh] overflow-hidden ' +
          (data.isSignature ? 'bg-white' : 'bg-slate-900')
        "
      >
        @if (data.photoUrl) {
          <img
            [src]="data.photoUrl"
            [class]="
              'max-w-full max-h-[60vh] object-contain rounded-xl ' +
              (data.isSignature
          ...
          <div
            class="w-full h-full min-h-[300px] border-2 border-dashed border-slate-700 rounded-xl flex items-center justify-center bg-slate-800"
          >
      ...
      <div class="p-4 border-t border-slate-100 flex justify-end gap-3 bg-slate-50">
        <button
          mat-stroked-button
          mat-dialog-close
          class="rounded-lg border-slate-300! text-slate-600!"
        >
          Cerrar
        </button>
        @if (data.canDelete !== false) {
          <button
            mat-flat-button
            color="warn"
            class="rounded-lg bg-rose-500! text-white"
...
export class PhotoPreviewDialogComponent {
  constructor(
    ...
    public data: { photoUrl: string; canDelete?: boolean; isSignature?: boolean },
  ) {}
}
